...
export const controlOptionsCreator = (qrcode: any) => {
  const errorQR = (setting: any, subcode: string) => {
    return { name: "QRoptionsError", setting, subcode };
  };
  const notInteger = (value: any) => value !== (value & ~0);

  // тест строки для кодирования text
  const testText = (text: any) => {
    try {
  ...
  const testMode = (mode: number, text: string) => {
    try {
      // регулярные выражения проверки mode
      const NUMERIC_REGEXP = /^\d*$/,
        ALPHANUMERIC_REGEXP = /^[A-Z0-9 $%*+\-./:]*$/;

      if (
        (mode === constQR.MODE_NUMERIC || mode === -1) &&
        !text.replace(NUMERIC_REGEXP, "")
      ) {
        mode = constQR.MODE_NUMERIC;
      } else if (
        (mode === constQR.MODE_ALPHANUMERIC || mode === -1) &&
        !text.replace(ALPHANUMERIC_REGEXP, "")
      ) {
  ...
  const testVersion = (version: number, ecclChoice: number) => {
    ...
    const ecclVersion = (version: number, ecclChoice: number, iStart = 0) => {
  ...
  try {
    ...
    // тест (подбор) версии qr-кода [с подбором уровеня корекции ошибок]
    testVersion(qrcode.version, (qrcode.eccl < 0) as any);
    // маска применяется (и выбирается) после формировиня qr-кода
    if (
      qrcode.mask !== -1 &&
      (qrcode.mask < 0 || qrcode.mask > 8 || notInteger(qrcode.mask))
    )
